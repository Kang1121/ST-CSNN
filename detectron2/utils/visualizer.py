        ...
        *,
        ...
        if boxes is not None:
            ...
            for i in range(boxes.shape[0]):
                if (boxes[i][0] < 10) | (boxes[i][2] > 1910) | (boxes[i][1] < 10) | (boxes[i][3] > 1070):
                    areas[i] = 0
                # print(labels[i])
                if labels[i] not in ['car', 'bus', 'truck', 'couch']:
                    areas[i] = 0
                # in different scenes, set a line, where don't need count
                # if ((boxes[i][2] * 1.1 + boxes[i][3]) > 1497) & ((boxes[i][2] * 0.31 + boxes[i][3]) < 454):
                #     areas[i] = 0
            # print(boxes[1])
            # print(areas[1])
        ...
        # first num_instances cannot be 0, otherwise output before
        # limit  num_instances  and recount from 1.
        if num_instances != 1:
            num_instances = 1
            for i in range(len(areas)):
                # compare with the biggest and whole picture
                # if (areas[i] > areas[sorted_idxs[0]] / 4) | (areas[i] > 1600 * 1200 / 48):
                if areas[i] > 1920 * 1080 / 90:
                    num_instances += 1
        f = open("../45b_3.txt", "a+")
